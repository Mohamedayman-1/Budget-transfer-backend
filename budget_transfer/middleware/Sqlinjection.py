...
class SQLInjectionProtectionMiddleware(MiddlewareMixin):
    ...
    def __init__(self, get_response):
        ...
        self.compiled_whitelist = [re.compile(pattern, re.IGNORECASE) for pattern in self.whitelist_patterns]
    ...
    def __call__(self, request):
        ...
        if request.path.startswith('/api/approvals/'):
            ...
            if self.contains_obvious_sql_injection(request):
                logger.warning(f"Obvious SQL injection attempt detected from {request.META.get('REMOTE_ADDR')}: {request.get_full_path()}")
    ...
    def check_json_data(self, data, context_path=None):
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                if self.is_malicious(str(key), context_path) or self.check_json_data(value, context_path):
    ...
    def contains_obvious_sql_injection(self, request):
        ...
        compiled_obvious = [re.compile(pattern, re.IGNORECASE) for pattern in obvious_patterns]
        
        # Check GET parameters
        for key, value in request.GET.items():
            for pattern in compiled_obvious:
                if pattern.search(str(value)):
                    logger.warning(f"Obvious SQL injection in GET parameter '{key}': {value}")
                    return True
